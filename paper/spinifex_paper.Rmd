...
One way to identify "interesting" features is with the use of a guided tour [@cook_grand_1995]. Guided tours select a very specific path, that which approaches a projection that optimizes an objective-function. The optimization is conducted in a manner similar to simulated annealing [@kirkpatrick_optimization_1983]. The direct optimization of a function allows guided tours to rapidly identify interesting projection features given the relatively large parameter-space. After a projection of interest is identified an analyst can then use the "finer brush" of the manual tour, by controlling the contributions of individual variables to explore the sensitivity they have on the structure visible in the projection.

<!-- Contributions 
This paper describes the algorithm used to perform a radial manual tour as implemented in the package \CRANpkg{spinifex}. It also describes how to generate an animation of the manual tour using the animation frameworks offered by \CRANpkg{plotly} [@sievert_plotly_2018] and \CRANpkg{gganimate} [@pedersen_gganimate:_2019]. The method is applied to multivariate data collected on high energy physics experiments (REF).
-->
...
This section describes the algorithm for performing a 2D radial manual tour, containing these steps:
...
- $\textbf{B} = (B_1,~ B_2)$, any 2D orthonormal projection basis, $p \times 2$ matrix, describing the projection from $p$ to two dimensions
- $\textbf{e}$, a 1D coordinate basis vector of length $p$ with the $k-$th element set to one, where $k$ is the index of the variable to manipulate, called the "manip var".
- $\theta_i$, the sequence of angles of in-projection-plane rotation, for example, on the reference axes.
- $\phi_i$, the sequence of angles of out-of-projection-plane rotation, coming into the manipulation space.
...
Call `view_basis()` on a basis to produce a \CRANpkg{ggplot2} [@wickham_ggplot2:_2016] graphic similar to Figure \ref{fig:step0}. The projected data is always available for display via matrix multiplication $\textbf{X}_{[n,~p]} ~*~ \textbf{B}_{[p,~d]} ~=~ \textbf{P}_{[n,~d]}$.
