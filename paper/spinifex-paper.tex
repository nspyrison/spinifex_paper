...
This section describes the algorithm for performing a 2D radial manual
tour, containing these steps:
...
each variable contributes to the projection. The right panel shows the
data as projected through the basis set described by the reference axes
(left). Data points are colored and given shape according to the
species.
...
similar to Figure \ref{fig:step0}. The projected data is always
available for display via matrix multiplication
